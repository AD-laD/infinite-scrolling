...
export default class Scene7 {
  ...
  createSceneObjects() {
    ...
    this.Ring = new ObjectModel3D('ring', { x: 1, y: 1, z: 1 }, this.scene)
    this.desk = new ObjectModel3D('desk', { x: 1, y: 1, z: 1 }, this.scene)
    this.desk.model.position.set(2.1,-1.5,0)
    this.desk.model.scale.set(10,10,10)

    const hourHandGeometry = new THREE.BoxGeometry(0.02, 0.15, 0.02);
    hourHandGeometry.translate(0, 0.075, 0);
    this.hourHand = new THREE.Mesh(
    hourHandGeometry,
    new THREE.MeshStandardMaterial({ color: 0x000000 })
    );
    this.hourHand.position.set(0, -0.07, 0.19); 
    this.scene.add(this.hourHand);

    const minuteHandGeometry = new THREE.BoxGeometry(0.01, 0.2, 0.02);
    minuteHandGeometry.translate(0, 0.1, 0); 
    this.minuteHand = new THREE.Mesh(
    minuteHandGeometry,
    new THREE.MeshStandardMaterial({ color: 0x000000 })
    );
    this.minuteHand.position.set(0, -0.07, 0.19); 
    this.scene.add(this.minuteHand);

    this.minuteHand.rotateX(-Math.PI/12)
    this.hourHand.rotateX(-Math.PI/12)

    this.handCenter = new THREE.Mesh(
        new THREE.SphereGeometry(0.025, 12, 12),
        new THREE.MeshStandardMaterial({ color: 0x000000 })
    );
    this.handCenter.position.set(0, -0.07, 0.18); 
    this.scene.add(this.handCenter)

    this.handCenter.rotateX(-Math.PI/12)
  ...
  resize() {
    ...
    update() {
        ...
        this.minuteHand.rotation.z = -(elapsedTime*100 / 60) * (Math.PI / 30); 
        this.hourHand.rotation.z = -(elapsedTime*100 / 3600) * (Math.PI / 6); 
